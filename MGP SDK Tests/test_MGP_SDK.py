...
class TestOgcMethods(unittest.TestCase):
    def setUp(self):
        ...
        self.basemap_ogc_filter = "productName='VIVID_STANDARD_30'"
    ...
    def test_streaming_download_image(self):
        ...
        result_wmts = self.interface.streaming.download_image(bbox=self.bbox,zoom_level=3, outputpath=f"{self.pwd}/Download.jpeg")
        self.assertIn("Download complete, files are located in",result_wmts)
        # exception tests
        with self.assertRaises(Exception):
    ...
    def test_basemap_download_image(self):
        ...
        self.assertIn('Downloaded file', result)
        # Basemap seamline
        result2 = self.interface.basemap_service.download_image(bbox=self.bbox, height=400, width=400, img_format='jpeg',
                                                                display=False, seamlines=True)
        self.assertIn('Downloaded file', result2)
        # exception tests
        with self.assertRaises(Exception):
    ...
    def test_basemap_search(self):
        # Basemap ogc
        self.assertEqual(type(self.interface.basemap_service.search(filter=self.basemap_ogc_filter, bbox=self.bbox)),
                         list)
        self.assertEqual(type(self.interface.basemap_service.search(bbox=self.bbox)), list)
        shapefile_search = self.interface.basemap_service.search(bbox=self.bbox, filter=self.basemap_ogc_filter,
                                                                 shapefile=True)
        csv_search = self.interface.basemap_service.search(bbox="39.84387,-105.05608,39.95133,-104.94827",
                                                           filter=self.basemap_ogc_filter, csv=True)
        self.assertEqual(type(shapefile_search), str)
        self.assertEqual(type(csv_search), str)
        self.assertTrue(os.path.isfile(shapefile_search))
        # Basemap seamline
        self.assertEqual(type(self.interface.basemap_service.search(filter=self.basemap_filter, bbox=self.bbox,
                                                                    seamlines=True)), list)
        self.assertEqual(type(self.interface.basemap_service.search(bbox=self.bbox, seamlines=True)), list)
        shapefile_seamline_search = self.interface.basemap_service.search(bbox=self.bbox, filter=self.basemap_filter,
                                                                          shapefile=True, seamlines=True)
        csv_seamline_search = self.interface.basemap_service.search(bbox="39.84387,-105.05608,39.95133,-104.94827",
                                                                    filter=self.basemap_filter, csv=True, seamlines=True)
        self.assertEqual(type(shapefile_seamline_search), str)
        self.assertEqual(type(csv_seamline_search), str)
        self.assertTrue(os.path.isfile(shapefile_seamline_search))
    ...
    def test_basemap_wmts(self):
        ...
        self.assertIn("Download complete", tile_download)
        # Basemap seamline
        tiles2 = self.interface.basemap_service.get_tile_list_with_zoom(bbox=self.bbox, zoom_level=13, seamlines=True)
        self.assertEqual(type(tiles2), list)
        tile2_download = self.interface.basemap_service.download_tiles(bbox=self.bbox, zoom_level=13, seamlines=True)
        self.assertIn("Download complete", tile2_download)
        with self.assertRaises(Exception):
...
class TestOrderingServiceMethods(unittest.TestCase):
    ...
    def test_order(self):
        ...
        order = self.interface.order_service.place_order(namespace, pipeline, output_config=output_config,
                                                         ...
                                                         metadata=metadata, endpoint='order', validate=False)
...
class TestTaskingServiceMethods(unittest.TestCase):
    def setUp(self):
        self.interface = Interface()
        self.one_day = (datetime.datetime.now() + datetime.timedelta(days=1)).strftime('%Y-%m-%dT%H:%M:%S+00:00')
        self.two_day = (datetime.datetime.now() + datetime.timedelta(days=2)).strftime('%Y-%m-%dT%H:%M:%S+00:00')
    ...
    def test_tasking(self):
        ...
        tasking = self.interface.tasking_service.new_tasking(start_datetime=self.one_day,
                                                             end_datetime=self.two_day,
...
class TestAnalyticsServiceMethods(unittest.TestCase):
    ...
    def test_raster_analytics(self):
        test_raster_url = self.interface.analytics.raster_url(script_id='ortho-image', function='ortho',
                                                     collect_id='10300100E538FE00',
                                                     api_token='ODI4YThiOWItODEzMS00NDczLWJhODQtNzBiOTU2YThlYTZj',
                                                     bands='nir1,red,green,blue', dra='true')
        self.assertEqual(type(test_raster_url), str)
        window = (3067, 2045, 2045, 2045)
        raster_arrays = self.interface.analytics.get_arrays(test_raster_url, *window)
        self.assertEqual(type(raster_arrays), list)
        raster_download = self.interface.analytics.download_raster(raster_array=raster_arrays, outputpath=os.path.join(os.getcwd(), "raster.png"))
        self.assertEqual(type(raster_download), str)
        self.assertTrue(os.path.isfile(os.path.join(os.getcwd(), 'raster.png')))
